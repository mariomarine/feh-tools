...
(defn snipes [session] (for [orb session :when (= (get orb "color") "Grey")] orb))
...
(defn -main
  "Simulate Summoning one color of heroes until 11 are obtained."
  [& args]
  (let [data (take 1000 (repeatedly #(snipes-2 0 0 0 BASE_CHANCES)))]
    (print-average! data)
    (print-min-max! data)
    (view (histogram data :nbins 100))
  )
)
